...
class ApiDashboardUserRolesResourceTestCase(unittest.TestCase):
    ...
    def test_dashboard_user_roles_resource_delete_without_token(self):
        response = self.client.delete(api.url_for(DashboardUserRolesResource, user_id=0, role_name='Role'), headers=self.get_headers())
        self.assertEqual(response.status_code,401)

    def test_dashboard_user_roles_resource_delete_with_valid_token_and_valid_user_id_and_role_name(self):
        r = Role(name='admin')
        r1 = Role(name='poweruser')
        u = User(username='test', email='test@test.com')
        u.set_password('test')
        ...
        token = json_data['token']
        user_id = json_data['user']['id']
        response = self.client.delete(api.url_for(DashboardUserRolesResource,user_id=user_id, role_name='poweruser'),
                                    data=json.dumps({}),
                                    headers=self.get_headers(token))
        self.assertEqual(response.status_code,204)

    def test_dashboard_user_roles_resource_delete_with_valid_token_and_role_and_valid_user_id_but_invalid_role_name(self):
        r = Role(name='admin')
        u = User(username='test', email='test@test.com')
        u.set_password('test')
        u.roles.append(r)
        ...
        token = json_data['token']
        user_id = json_data['user']['id']
        response = self.client.delete(api.url_for(DashboardUserRolesResource,user_id=user_id, role_name='Role'),
                                    data=json.dumps({}),
                                    headers=self.get_headers(token))
        self.assertEqual(response.status_code,404)

    def test_dashboard_user_roles_resource_delete_with_valid_token_and_role_and_invalid_user_id_and_valid_role_name(self):
        r = Role(name='admin')
        u = User(username='test', email='test@test.com')
        u.set_password('test')
        u.roles.append(r)
        ...
        token = json_data['token']
        user_id = json_data['user']['id']
        response = self.client.delete(api.url_for(DashboardUserRolesResource,user_id=user_id+1, role_name='admin'),
                                    data=json.dumps({}),
                                    headers=self.get_headers(token))
        self.assertEqual(response.status_code,404)

    def test_dashboard_user_roles_resource_delete_with_valid_token_and_role_and_invalid_user_id_and_role_name(self):
        r = Role(name='admin')
        u = User(username='test', email='test@test.com')
        u.set_password('test')
        u.roles.append(r)
        ...
        token = json_data['token']
        user_id = json_data['user']['id']
        response = self.client.delete(api.url_for(DashboardUserRolesResource,user_id=user_id+1, role_name='Role'),
                                    data=json.dumps({}),
                                    headers=self.get_headers(token))
        self.assertEqual(response.status_code,404)
    ...
    def test_dashboard_user_roles_resource_delete_with_invalid_token(self):
        response = self.client.delete(api.url_for(DashboardUserRolesResource,user_id=0,role_name='Role'), headers=self.get_headers('THIS_IS_AN_INVALID_TOKEN'))
        self.assertEqual(response.status_code,401)
